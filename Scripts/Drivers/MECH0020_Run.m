function MECH0020_Run(varargin)
% MECH0020_Run - Unified entry point for MECH0020 Tsunami Vortex Simulation
%
% Purpose:
%   Single combined driver merging Analysis.m and Tsunami_Simulator.m
...
%
% Usage:
%   MECH0020_Run()                          % Interactive: shows startup dialog
%   MECH0020_Run('Mode', 'UI')              % Direct UI launch
%   MECH0020_Run('Mode', 'Standard')        % Direct Standard mode (defaults)
...
%
% Outputs:
%   None (results saved to Results/ directory)
%
% See also: Analysis, Tsunami_Simulator, UIController, ModeDispatcher
...
function run_interactive_mode(opts)
    ColorPrintf.header('MECH0020 TSUNAMI VORTEX SIMULATION');
...
function run_ui_mode()
    ColorPrintf.header('MECH0020 TSUNAMI VORTEX SIMULATION - UI MODE');
    app = UIController();  %#ok<NASGU>
...
function run_standard_mode(opts)
    ColorPrintf.header('MECH0020 TSUNAMI VORTEX SIMULATION - STANDARD MODE');
