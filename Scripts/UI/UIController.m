...
classdef UIController < handle
    ...
    methods
        function app = UIController(varargin)
            ...
            % CRITICAL: Suppress ALL traditional MATLAB figure windows
            % The UI uses uifigure exclusively - block any figure() calls
            ...
            app.figures_list = {};  % Cell array of figure data structs
            ...
            app.layout_cfg = app.UI_Layout_Config();
        ...
        function create_monitoring_tab(app)
            ...
            for tile_idx = 1:cfg.plot_tile_count
                ...
                app.style_axes(ax);
                title(ax, metric.title, 'Color', C.fg_text, 'FontSize', 14, 'FontWeight', 'bold', 'Interpreter', 'latex');
                xlabel(ax, metric.xlabel, 'Color', C.fg_text, 'FontSize', 11, 'Interpreter', 'latex');
                ylabel(ax, metric.ylabel, 'Color', C.fg_text, 'FontSize', 11, 'Interpreter', 'latex');
                grid(ax, 'on');
                ax.GridAlpha = 0.3;
                ax.FontSize = 10;
                ax.XColor = C.fg_text;
        ...
        function launch_simulation(app)
            ...
            % Ensure figures stay suppressed during simulation runs
            % (DefaultFigureVisible is already 'off' from constructor, but reinforce it here)
            set(0, 'DefaultFigureVisible', 'off');
            ...
            try
                ...
                % Don't auto-switch to monitoring tab - let user stay on current tab
                ...
                app.capture_run_figures(figs_before, summary);
                ...
                app.capture_run_figures(figs_before, struct());
            ...
            % Keep figures suppressed - UI mode doesn't use traditional figures
            % (Don't restore DefaultFigureVisible - keep it 'off' permanently in UI mode)
        end
        ...
        function collect_configuration_from_ui(app)
            ...
            app.config.ic_type = app.map_ic_display_to_type(ic_display_name);
        ...
        function summary = execute_single_run(app, cfg_override)
            ...
            % Wait for completion (UI updates via independent timer)
            [results, paths] = app.current_executor.wait_for_completion();
        ...
        function update_system_metrics_display(app, payload)
            ...
            % Update elapsed time
            if isfield(payload, 'elapsed_time') && isfinite(payload.elapsed_time)
        ...
        function export_configuration(app)
            ...
            config_export.ic_type = app.map_ic_display_to_type(app.handles.ic_dropdown.Value);
        ...
        function update_terminal_from_diary(app)
            ...
            try
                ...
                % Filter out noisy MATLAB internal output (useCS, ans =, etc.)
                ...
                for i = 1:numel(new_lines)
                    ...
                    for np = 1:numel(noise_patterns)
                        if contains(line_str, noise_patterns(np))
                            is_noise = true;
                            break;
                        end
                    end
        ...
        function add_figure(app, fig, name)
            ...
            % Capture the figure frame before storing (since it will be closed later)
            try
                % Make figure visible temporarily to capture its content
                ...
                % Store the captured frame data instead of the figure handle
                fig_data = struct('frame', frame, 'name', name, 'handle', fig);
                app.figures_list{end+1} = fig_data;
            catch ME
                % Fallback: store the figure handle if frame capture fails
        ...
        function refresh_figures(app)
            ...
            % Build figure names from stored data
            fig_names = cell(1, length(app.figures_list));
            for i = 1:length(app.figures_list)
                if ~isempty(app.figures_list{i}.name)
                    fig_names{i} = app.figures_list{i}.name;
                else
        ...
        function show_figure(app, idx)
            ...
            try
                ...
                % Use the pre-captured frame from the stored figure data
                fig_data = app.figures_list{idx};
                if ~isempty(fig_data.frame)
                    image(ax, fig_data.frame.cdata);
                    axis(ax, 'off');
                else
                    % Fallback: try to capture from handle if frame wasn't captured
                    if isvalid(fig_data.handle)
                        frame = getframe(fig_data.handle);
                        image(ax, frame.cdata);
                        axis(ax, 'off');
                    end
                end
            catch
                % If frame display fails, keep placeholder
            end
        end
            ...
            for idx = 1:numel(keys)
                ...
                if app.has_valid_handle(key)
                    try
                        ...
                        % Some components may not support Tooltip property
                    end
                end
            end
        end
...
%                'rank', num2cell([1, 2, 5, 3, 4, 6, 8, 9, 10, 11, 12, 7]));
        ...
        function apply_monitor_plot_format(app, ax, metric)
            ...
            title(ax, metric.title, 'Color', app.layout_cfg.colors.fg_text, 'FontSize', 14, 'FontWeight', 'bold', 'Interpreter', 'latex');
            xlabel(ax, metric.xlabel, 'Color', app.layout_cfg.colors.fg_text, 'FontSize', 11, 'Interpreter', 'latex');
            y_interp = 'latex';
            if strcmp(metric.id, 'cpu_proxy') || strcmp(metric.id, 'iteration_completion')
                y_interp = 'none';
            end
            ylabel(ax, metric.ylabel, 'Color', app.layout_cfg.colors.fg_text, 'FontSize', 11, 'Interpreter', y_interp);
    ...
    methods (Access = private)
        ...
        function cfg = UI_Layout_Config(app)
            ...
            cfg.config_tab.grid_quad.row_heights   = {'1x', '1x'};
            ...
            cfg.monitor_tab.live_update = struct( ...
                'refresh_stride', 1, ...          % Refresh plots every N accepted payloads (increased for performance)
                'min_refresh_seconds', 0.01, ...    % Time-based refresh floor (0.1s = 10 updates/sec max)
                'force_refresh_every', 10, ...    % Guaranteed refresh cadence (increased for performance)
                'max_history_points', 0);        % Keep tail samples for responsive plotting
            all_methods = {'finite_difference', 'finite_volume', 'spectral'};
            all_modes = {'evolution', 'convergence', 'sweep', 'animation', 'experimentation'};
            conv_only = {'convergence'};
            ...
            cfg.ui_text.window = struct( ...
                ...
                'menu_title', 'Tsunami Vortex Simulation');
            ...
            cfg.ui_text.config.root = struct( ...
                'left_panel_title', 'Simulation Settings', ...
            ...
            cfg.ui_text.config.method = struct( ...
                ...
                'method_label', 'Method', ...
                'mode_label', 'Mode', ...
                'boundary_label', 'Boundary Conditions', ...
                ...
                'bathy_browse_button', 'Import Bathymetry', ...
            ...
            cfg.ui_text.monitor.sidebar = struct( ...
                ...
                'collector_panel_title', 'Collector & Signals', ...
            ...
            cfg.ui_options.mode_items = {'Evolution', 'Convergence', 'Parameter Sweep', 'Animation', 'Experimentation'};
            ...
            cfg.ui_options.ic_type_items = {'Stretched Gaussian', 'Vortex Blob', 'Vortex Pair', 'Multi-Vortex', 'Lamb-Oseen', 'Rankine', 'Lamb Dipole', 'Taylor-Green', 'Random Turbulence', 'Elliptical Vortex'};
            ...
            cfg.ui_defaults.save_mat = false;
            ...
            cfg.math_tokens = struct( ...
                ...
                'vorticity', '$\omega$', ...
                'streamfunction', '$\psi$', ...
                'omega_max', '$|\omega|_{max}$', ...
                'omega_abs', '$|\omega|$', ...
                'omega_norm', '$\|\omega\|$', ...
                'partial_omega_partial_t', '$\partial\omega/\partial t$', ...
                'partial2_omega_partial_x2', '$\partial^2\omega/\partial x^2$', ...
                'partial2_omega_partial_y2', '$\partial^2\omega/\partial y^2$', ...
                'nabla2_omega', '$\nabla^2\omega$', ...
                'energy', '$E$', ...
                'enstrophy', '$Z$', ...
                'Nx', '$N_x$', ...
            ...
            cfg.tooltips = struct( ...
                ...
                'conv_n_max', sprintf('Finest grid resolution for convergence study.\nMaximum mesh density to test.'), ...
                'bathy_enable', sprintf('Enable bathymetry (variable ocean floor depth).\nOnly available for Finite Difference method.'), ...
                'motion_enable', sprintf('Enable seabed motion forcing.\nSimulates tectonic plate movement or wave generators.'), ...
                'motion_amplitude', sprintf('Amplitude of seabed motion (metres).\nControls strength of bottom forcing term.'));
