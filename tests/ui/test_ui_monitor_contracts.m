...
% test_ui_monitor_contracts - Regression checks for UI monitor/layout contracts.
    ...
    try
        ...
        assert(isfield(app.handles, 'config_left_subtab_group') && isvalid(app.handles.config_left_subtab_group), ...
            'config_left_subtab_group handle is missing.');
        assert(numel(app.handles.config_left_subtab_group.Children) >= 6, ...
            'Configuration left tabgroup must expose method/grid/time/simulation/convergence/sustainability subtabs.');
        assert(isfield(app.handles, 'label_Nx') && isvalid(app.handles.label_Nx) && ...
            strcmpi(string(app.handles.label_Nx.Interpreter), "latex"), ...
            'Nx label must use LaTeX interpreter.');
        assert(isfield(app.handles, 'label_dt') && isvalid(app.handles.label_dt) && ...
            strcmpi(string(app.handles.label_dt.Interpreter), "latex"), ...
            'dt label must use LaTeX interpreter.');
        assert(isfield(app.handles, 'time_video_axes_map') && isstruct(app.handles.time_video_axes_map) && ...
            ...
            'Time/Physics triplet must expose MP4/AVI/GIF axes handles.');
        assert(isfield(app.handles, 'btn_time_video_play') && isvalid(app.handles.btn_time_video_play) && ...
            ...
            'Triplet transport controls (Play/Pause/Restart/Load Latest) must exist.');
